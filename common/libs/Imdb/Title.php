...
class Title extends MdbBase
{
    ...
    protected $related_titles = array();
    ...
    protected function title_year()
    {
        ...
        if (@preg_match('!<title>(IMDb\s*-\s*)?(?<ititle>.*)(\s*-\s*IMDb)?</title>!', $this->page["Title"], $imatch)) {
            ...
            if (preg_match('/<h1.+?>(.+?)<\/h1>/m', $this->page["Title"], $h1_title)) {
                $this->main_title = html_entity_decode($h1_title['1'],  ENT_QUOTES|ENT_HTML5, "UTF-8");
            }
            if ($this->main_year == "????") {
                $this->main_year = "";
            }
    ...
    public function seasons()
    {
        if ($this->seasoncount == -1) {
            $this->getPage("Title");
            $html = (new SimpleHtmlDom())->str_get_html($this->page["Title"]);
            $items = $html->find('.episodes-browse-episodes > div:nth-of-type(2) > *');

            $season_tag = !empty($items['1']) ? $items['1']->tag : null;

            switch ($season_tag) {
                case 'a':
                    $label = $items['1']->find('.ipc-button__text')['0']->innertext;
                    break;
                case 'div':
                    $label = $items['1']->find('label');
                    $label = $label['0']->innertext;
                    break;
                default:
                    $label = null;
                    break;
            ...
            preg_match_all('/(\d+)\s[sS]eason[s]?/m', $label, $matches);

            $this->seasoncount = intval(ArrayHelper::getValue($matches, '1.0', '0'), 10);
        }
    ...
    public function cast($short = false)
    {
        ...
        if (empty($page)) {
            return $this->credits_cast; // no such page
        }

        $this->parseTitleCastGrid($page);
    ...
    public function getRelated()
    {
        ...
        if (!empty($this->related_titles)) {
            return $this->related_titles;
        }

        // Create DOM from URL
        $simple_html_dom = new SimpleHtmlDom();
        $html = $simple_html_dom->str_get_html($page);

        $related_box = null;
        // Start Related Shows Loop
        foreach ($html->find('section.ipc-page-section') as $section) {
            $h3 = $section->find('h3.ipc-title__text');
            if (!empty($h3['0']) && $h3['0']->innertext === 'More like this') {
                $related_box = $section;
                break;
            }
        }

        if (empty($related_box)) {
            return $this->related_titles;
        }

        foreach ($related_box->find('a.ipc-lockup-overlay') as $related_title) {
            $imdb_id = preg_replace('/\/title\/(tt[0-9]+)\/.+/m', "$1", $related_title->href);
            if (!empty($imdb_id))
                $this->related_titles[] = $imdb_id;
        }
     ...
     * @see IMDB page /fullcredits
     */
    public function castDeprecated($short = false)
    {
        if (!empty($this->credits_cast)) {
            return $this->credits_cast;
        }
  ...
  */
     ...
     * @return array
     */
    protected function parseTitleCastGrid($page = null)
    {
        $simple_html_dom = new SimpleHtmlDom();
        if (empty($page)) {
            return [];
        ...
        $document_dom = $simple_html_dom->str_get_html($page);
        $cast_grid = $document_dom->find('.title-cast__grid .ipc-shoveler__grid > div');
        foreach ($cast_grid as $div) {
            $dir = array(
                'imdb' => null,
                'name' => null,
                'name_alias' => null,
            ...
            $img = $div->find('img');
            $href = $div->find('.title-cast-item__avatar + div > a');
            $char_span = $div->find('.title-cast-item__characters-list span');

            $dir['thumb'] = !empty($img['0']) ? $img['0']->src : null;
            if (!empty($dir['thumb']))
                $dir['photo'] = preg_replace('#\._V1_.+?(\.\w+)$#is', '$1', $dir["thumb"]);

            $dir["imdb"] = !empty($href['0']) ? preg_replace('/\/name\/nm(\d+).+/m', '$1', $href['0']->href) : null;
            $dir["name"] = !empty($href['0']) ? trim(strip_tags($href['0']->innertext)) : null;
            $dir["character"] = !empty($char_span['0']) ? trim(strip_tags($char_span['0']->innertext)) : null;

            if (!empty($dir['name']) && !empty($dir['imdb'])) {
                $this->credits_cast[] = $dir;
            }
        }
    }
